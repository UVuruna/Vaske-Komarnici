<div id="footer">
    <section id="contact">
        <h2>Kontaktirajte nas</h2>
        <p>
            Za više informacija o našim proizvodima i uslugama, ili da rezervišete
            termin za merenje, slobodno nas kontaktirajte putem sledećih kanala:
        </p>
        <ul>
            <li>Telefon: <strong>+381 63 1051331</strong></li>
            <li>Email: <strong>vaske.komarnici@gmail.com</strong></li>
            <li>Adresa: <strong>Beograd, Srbija</strong></li>
        </ul>
    </section>

    <footer>
        <p>&copy; <?php echo date("Y"); ?> Vaske Komarnici | Sva prava zadržana</p>
        <p>Autor: UVS — Uros Vuruna</p>
        <p>Kontakt: <a href="mailto:vurunayas@gmail.com">vurunayas@gmail.com</a></p>
    </footer>
</div>